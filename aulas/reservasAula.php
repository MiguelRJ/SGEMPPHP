...
if (isset($_GET['_id'])){
    ...
    if (isset($_POST['id'])){
        ...
        $id = null;
        header('Location: buscarAula.php');
            ...
            <div class="col-7">
                ...
                echo '<table class="table table-hover table-dark table-striped">
                    ...
                    <tbody>';
                        ...
                        foreach ($books as $row) { // substr($row["name"] , 0, 30) solo muestra los primeros 30 caracteres
                            echo '
                            <tr>
                                ...
                                <th class="text-center" scope="row">';
                                    ...
                                    if ($row["bookReason"] != null) {
                                        echo '<button class="btn btn-outline-secondary" onclick="showBookReason(\''.$row["bookReason"].'\')">
                                ...
                                echo'</th> <th class="text-center align-middle" scope="row">';
                                    
                                    if ($row["bookReason"] == null) {
                                        echo '<button class="btn btn-outline-secondary" onclick="confirmBook('.$app->getDao()->getUserIdByName($_SESSION['user']).','.$id.','.$row["_id"].','.str_replace("-","",$date).')">
